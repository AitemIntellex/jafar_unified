...
from src.jafar.utils.gemini_api import ask_gemini_with_image, ask_gemini_text_only
from src.jafar.utils.news_api import get_news
from src.jafar.utils.topstepx_api_client import TopstepXClient
from src.jafar.cli.telegram_handler import send_telegram_media_group, send_long_telegram_message
from src.jafar.cli.economic_calendar_fetcher import fetch_economic_calendar_data
...
def run_atrade_analysis(instrument_query: str, contract_id: str, screenshot_files: list[str]) -> str:
    ...
    prompt = f"""Симуляция. Роль: опытный интрадей-трейдер. Анализируемый инструмент: {instrument_query}.
    Задача: разработать детальный и гибкий торговый план на 2-4 часа.
    Входные данные: 3 скриншота, новости, экономический календарь и РЕАЛЬНЫЕ ДАННЫЕ С ТОРГОВОГО СЧЕТА.

    **ДАННЫЕ С ТОРГОВОГО СЧЕТА (TopstepX API):**
    ```{topstepx_data}```

    **НОВОСТИ ({instrument_query}):**
    ```{news_results}```
    ...
    **ЗАДАНИЕ:**
    ...
    1.  **Общий Анализ:** Проанализируй **ВСЕ** источники, включая данные со счета. Определи основной тренд, сентимент и ключевые уровни. Если на счете уже есть открытая позиция, ОБЯЗАТЕЛЬНО учти ее в своем анализе. Оцени уверенность в прогнозе (A, B, C).

    2.  **Основной План (План А):**
        *   Сформулируй основной торговый план: **Действие** (Покупка/Продажа), **Точка Входа**, **Stop-Loss**. 
        *   Если уже есть позиция, план должен быть по ее управлению. Если план — закрыть существующую Long позицию, используй `"action": "SELL_TO_CLOSE"`. Если закрыть Short — `"action": "BUY_TO_CLOSE"`.

    3.  **Управление Позицией:**
        *   **Цели (Take-Profit):** Определи две цели. **TP1** для частичной фиксации и переноса стопа в безубыток, и **TP2** как основную цель.

    4.  **Альтернативный Сценарий (План Б):**
        *   Опиши краткий план действий, если цена пойдет против основного сценария.

    5.  **Голосовая Сводка:**
        *   Сгенерируй очень краткую сводку (2-3 предложения) для голосового ассистента, озвучив только **основной план (План А)**.

    6.  **JSON Вывод:**
        *   Включи в самый конец ответа JSON со всеми числовыми значениями. Для `action` используй "BUY", "SELL", "SELL_TO_CLOSE" или "BUY_TO_CLOSE".
    ...
    **ПРИМЕР JSON ВЫВОДА:**
    ...
    try:
        ...
        # --- Разделение текста и JSON для красивого вывода ---
        json_string = None
        text_analysis_russian = analysis_result
        voice_summary_russian = None
        trade_data = None

        # Ищем и извлекаем JSON блок
        ...
        if json_match:
            ...
            # Удаляем JSON блок из основного текста для чистого вывода
            text_analysis_russian = analysis_result.replace(json_match.group(0), "").strip()
            try:
                trade_data = json.loads(json_string)
                voice_summary_russian = trade_data.get("voice_summary")
            except json.JSONDecodeError:
                console.print("[red]Ошибка декодирования JSON. Вывод сырого блока:[/red]")
                console.print(json_string)
        else:
            console.print("[yellow]JSON блок с данными не найден в ответе.[/yellow]")

        # --- Перевод текста на узбекский (напрямую в кириллице) ---
        console.print("\n[blue]Анализни ўзбек тилига (кирилл алифбосида) таржима қиляпман...[/blue]")
        text_analysis_uzbek_cyrillic = ask_gemini_text_only(f"Переведи следующий текст на узбекский язык (кириллицада):\n\n{text_analysis_russian}")
        
        voice_summary_uzbek_cyrillic = None
        if voice_summary_russian:
            # Уточняем промпт, чтобы Gemini не добавлял лишние комментарии и сразу давал кириллицу
            voice_summary_uzbek_cyrillic = ask_gemini_text_only(f"Переведи только следующий текст на узбекский язык (кириллицада, без лишних комментариев):\n\n{voice_summary_russian}")
            if trade_data:
                trade_data["voice_summary"] = voice_summary_uzbek_cyrillic
                json_string = json.dumps(trade_data, indent=2, ensure_ascii=False) # Обновляем JSON строку
        ...
        if voice_summary_uzbek_cyrillic:
            ...
            speak_mac_text(voice_summary_uzbek_cyrillic)
        else:
        ...
        if trade_data:
            try:
                ...
                if action in ["BUY", "SELL"] and trade_data.get("primary_entry"):
                    ...
                    confirmation = console.input("[bold]Ушбу ордерни жойлаштирайми? (ҳа/1 ёки йўқ/0): [/bold]").lower()
        ...
        # Отправляем ПОЛНЫЙ оригинальный результат в Telegram (на узбекском, кириллица)
        telegram_message_uzbek_cyrillic = f"{text_analysis_uzbek_cyrillic}\n\n```json\n{json_string}\n```" if json_string else text_analysis_uzbek_cyrillic
        send_telegram_media_group(screenshot_files, "Супер-комплексный анализ (ўзбек тилида).", parse_mode="MarkdownV2")
        send_long_telegram_message(telegram_message_uzbek_cyrillic, parse_mode="MarkdownV2")
...
def atrade_command(args: str = None):
    ...
    for i in range(3):
        console.print(f"\n[cyan]Скриншот #{i + 1}/3 учун тайёрланинг (5 сония)...[/cyan]")
        time.sleep(5)
