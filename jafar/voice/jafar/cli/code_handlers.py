...
- `code refactor <файл> [инструкция]` — AI рефакторит код в файле
- `code review <файл>` — AI анализирует код в файле
...
- `code refactor my_script.py "Оптимизируй циклы"`
- `code review another_script.py`
...
def edit_code(args: str):
    ...
    prompt = f"Предложи улучшения к этому коду и верни ТОЛЬКО обновлённый вариант кода, без лишних слов и пояснений, в формате Markdown code block (```python\n...\n```):\n\n```python\n{content[:3000]}\n```"
    response = ask_assistant(prompt)
    ...
    if isinstance(response, dict):
        # Приоритет: сначала 'command', потом 'message', потом 'explanation'
        raw_response = response.get("command") or response.get("message") or response.get("explanation", "")
    ...
    # Извлечение кода из Markdown code block
    code_match = re.search(r"```(?:python|py)?\n(.*?)\n```", raw_response, re.DOTALL)
    if code_match:
        code_text = code_match.group(1).strip()
    else:
        # Если нет блока кода, используем весь ответ как код (с предупреждением)
        code_text = raw_response.strip()
        console.print("[yellow]Предупреждение: Ответ AI не содержит блока кода. Используется весь ответ как код.[/yellow]")

    if not code_text:
        console.print("[red]❌ AI не вернул код для редактирования.[/red]")
        return
...
def edit_file_by_path(filepath: str):
    ...
    prompt = f"""Файл вызывает ошибки. Предложи улучшения и верни ТОЛЬКО исправленный код, без лишних слов и пояснений, в формате Markdown code block (```python\n...\n```):\n```python\n{content[:3000]}\n```"""
...
def refactor_code(args: str):
    parts = args.split(maxsplit=2)
    if len(parts) < 2:
        console.print("[yellow]Укажи файл: code refactor <имя.py> [инструкция][/yellow]")
        return
    file_path = parts[1]
    instruction = parts[2] if len(parts) > 2 else "Улучши читаемость и производительность."
    full_path = os.path.join(get_projects_root(), file_path)
    if not os.path.exists(full_path):
        console.print(f"[red]Файл не найден: {file_path}[/red]")
        return
    ...
    prompt = f"""Рефакторинг кода: {instruction}

Верни ТОЛЬКО рефакторинговый код, без лишних слов и пояснений, в формате Markdown code block (```python
...
```):
```python
{content[:3000]}
```"""
    ...
    if not code_text:
        console.print("[red]❌ AI не вернул код для рефакторинга.[/red]")
        return

    console.print(
        Panel(
            Markdown(f"```python\n{code_text}\n```"),
            title=f"✨ Рефакторинг: {file_path}",
            style="green",
        )
    )
...
def review_code(args: str):
    parts = args.split(maxsplit=1)
    if len(parts) < 2:
        console.print("[yellow]Укажи файл: code review <имя.py>[/yellow]")
        return
    file_path = parts[1]
    full_path = os.path.join(get_projects_root(), file_path)
    if not os.path.exists(full_path):
    ...
    prompt = f"""Проанализируй следующий код на предмет потенциальных проблем, улучшений, ошибок или объяснений. Верни ТОЛЬКО результат анализа, без лишних слов и пояснений, в формате Markdown:
```python
{content[:3000]}
```"""

    response = ask_assistant(prompt)
    review_text = response.get("message") or response.get("explanation") or str(response)

    console.print(
        Panel(
            Markdown(review_text),
            title=f"🧐 Обзор кода: {file_path}",
            style="blue",
        )
    )
